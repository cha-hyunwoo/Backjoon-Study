...
문제에서 찾는 게 만들 수 있는 문자열의 조합이 아니라 그냥 나와 있는 상태의 문자 중에 잘라서 어떤 문자열, 문자들이 나오는지 찾는 거라 HashSet으로 나오는 문자열 substring(i부터 j전까지 나오는 문자열)으로 다 넣고 set.size로 몇 개 들어가 있는지 찾아서 해결했음 

### 제출 일자

2026년 2월 20일 15:57:55
